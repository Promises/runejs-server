...
import {
    Appearance,
    authenticatePlayer, createAccount,
    defaultAppearance,
    defaultSettings,
    FBResponseCode,
    loadPlayerSaveFirebase,
    PlayerSave,
    PlayerSettings,
    QuestProgress,
    savePlayerData, savePlayerDataFirebase
} from './player-data';
...
export class Player extends Actor {
    ...
    public fbCredentials: firebase.auth.UserCredential;
    ...
    public handleAuth(): Promise<boolean> {
        return authenticatePlayer(this.username, this.password).then((async ([credentials, response]) => {
                switch (response) {
                    case FBResponseCode.EXISITNG_USER:
                        this.fbCredentials = credentials;
                        return true;
                    case FBResponseCode.NEW_USER:
                        await createAccount(this.username, this.password).then(([credentials, response]) => {
                            this.fbCredentials = credentials;
                        });
                        return true;
                    case FBResponseCode.WRONG_CREDENTIALS:
                        return false
                }
    ...
    public async loadSaveData(): Promise<void> {
        const playerSave: PlayerSave = await loadPlayerSaveFirebase(this.fbCredentials);
